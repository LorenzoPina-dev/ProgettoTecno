 ...
 * @author user
 */
public class ThreadRicercaProv extends Thread {

    private String FileRicerca, AttributoRicerca;
    private int indice;

    public ThreadRicercaProv(String FileRicerca, String AttributoRicerca, int indice) {
    ...
    public void run() {
        Vector<String> righe = GestoreProvincia.Instance().getFile(FileRicerca);
        String[] campi = righe.get(0).split(";");
        int Att = -1, Provincia = -1;
        ...
        while (campi.length >= i && (!trovato1||!trovato2)) {
            ...
            if (campi[i].equals("denominazione_provincia") || campi[i].equals("nome_area")) {
                Provincia = i;
                trovato2=true;
            }
            i++;
        ...
        for (i = 1; i < righe.size(); i++) {
            ...
            if (attributi[Att] == null) {
                ...
                GestoreProvincia.Instance().settaValore(attributi[Provincia], indice, attributi[Att]);
            }
        }
    }
}